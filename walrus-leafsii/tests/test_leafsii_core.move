...
module leafsii::test_leafsii_core {
    ...
    fun setup_protocol_with_pool(
        ...
        clock: &Clock
    ): (coin::Coin<TEST_FTOKEN>, coin::Coin<TEST_XTOKEN>, leafsii::AdminCap) {
        ...
        (coin_f, coin_x, admin_cap)
    }

    #[test]
    fun test_protocol_initialization() {
        let (mut scenario, clock, oracle) = setup_test();
        let (coin_f, coin_x, admin_cap) = setup_protocol_with_pool(&mut scenario, &clock);
        ...
        assert!(nf == 1_000, 0); // Changed from 1_000_000 due to PF_FIXED = 1e9
        assert!(nx == 500_000, 1);
        assert!(pf == 1_000_000_000, 2); // $1.00 in nano-USD (1e9), not micro-USD
    ...
    fun test_protocol_initialization_with_reserve_50_50() {
        ...
        // Expect 50/50 USD split: $1.0 to F -> 0.001 F minted (nano-USD); $1.0 to X -> 0.5 X minted at px=$2.0
        let (nf, nx, pf, px, p_r, reserve, _, _) = leafsii::get_protocol_state<TEST_FTOKEN, TEST_XTOKEN>(&protocol);
        assert!(pf == 1_000_000_000, 0); // Changed from 1_000_000, now in nano-USD (1e9)
        assert!(p_r == INITIAL_PRICE_E6, 1);
        assert!(reserve == DEPOSIT_AMOUNT, 2);
        assert!(px == INITIAL_PRICE_E6 / 1, 3);
        assert!(nf == 1_000, 4); // Changed from 1_000_000 due to PF_FIXED = 1e9
    ...
    fun test_mint_and_redeem_f_tokens() {
        ...
        ts::next_tx(&mut scenario, @0x1);
        let mut protocol = ts::take_shared<leafsii::Protocol<TEST_FTOKEN, TEST_XTOKEN>>(&scenario);
        let pool = ts::take_shared<stability_pool::StabilityPool<TEST_FTOKEN>>(&scenario);
        // Using admin_cap from init_protocol
        ...
        // Test minting F tokens - with 0.5% fee, user gets 1,990 F tokens (changed due to PF_FIXED = 1e9)
        let coin_f2 = leafsii::mint_f<TEST_FTOKEN, TEST_XTOKEN>(&mut protocol, &pool, test_coin, ts::ctx(&mut scenario));
        assert!(&coin_f2.value() == 1_990, 0); // Changed from 1_990_000 due to PF_FIXED = 1e9
        ...
        assert!(nf_supply == 2_990, 0); // Changed from 2_990_000 due to PF_FIXED = 1e9
        ...
        let (coin_r, ticket_opt) = leafsii::redeem_f<TEST_FTOKEN, TEST_XTOKEN>(&mut protocol, &pool, coin_f2, &clock, ts::ctx(&mut scenario));
    ...
    fun test_mint_and_redeem_x_tokens() {
        ...
        let (coin_r, ticket_opt) = leafsii::redeem_x(&mut protocol, &pool, x_coin, &clock, ts::ctx(&mut scenario));
        option::destroy_none(ticket_opt);
