...
module leafsii::test_stability_pool_rewards {
    ...
    fun test_sp_reward_accrual_single_user() {
        ...
        let (burned, indexed) = stability_pool::sp_controller_rebalance(&mut pool, &dummy_cap, f_burn_amount, r_reward_amount);
    ...
    fun test_sp_reward_claiming() {
        ...
        let extra_reserve = coin::mint_for_testing<SUI>(1_000_000, ts::ctx(&mut scenario));
        let _minted = leafsii::mint_f(&mut protocol, &pool, extra_reserve, ts::ctx(&mut scenario));
        coin::burn_for_testing(_minted);
        ...
        let pool_protocol_id = stability_pool::pool_id(&pool);
        let dummy_cap = stability_pool::create_dummy_capability_with_id(pool_protocol_id, ts::ctx(&mut scenario));
        let (_burned, _indexed) = stability_pool::sp_controller_rebalance(&mut pool, &dummy_cap, 400_000, reward_amount);
        ...
        // Claim rewards
        let claimed_reserve = leafsii::claim_sp_rewards(&mut protocol, &mut pool, &mut position, ts::ctx(&mut scenario));
    ...
    fun test_multiple_users_reward_distribution() {
        ...
        let mut position1 = stability_pool::create_position<TEST_FTOKEN>(ts::ctx(&mut scenario));
        ...
        let pool_protocol_id = stability_pool::pool_id(&pool);
        let dummy_cap = stability_pool::create_dummy_capability_with_id(pool_protocol_id, ts::ctx(&mut scenario));
        let (_burned, _indexed) = stability_pool::sp_controller_rebalance(&mut pool, &dummy_cap, 500_000, total_rewards);
        ...
        let claimed1 = leafsii::claim_sp_rewards(&mut protocol, &mut pool, &mut position1, ts::ctx(&mut scenario));
        ...
        let claimed2 = leafsii::claim_sp_rewards(&mut protocol, &mut pool, &mut position2, ts::ctx(&mut scenario));
    ...
    fun test_partial_liquidation_rewards() {
        ...
        // Add extra reserve
        let extra_reserve = coin::mint_for_testing<SUI>(1_500_000, ts::ctx(&mut scenario));
        let _minted = leafsii::mint_f(&mut protocol, &pool, extra_reserve, ts::ctx(&mut scenario));
        coin::burn_for_testing(_minted);
        ...
        let (_burned1, _indexed1) = stability_pool::sp_controller_rebalance(&mut pool, &dummy_cap1, 400_000, reward1);
        ...
        let (_burned2, _indexed2) = stability_pool::sp_controller_rebalance(&mut pool, &dummy_cap2, 300_000, reward2);
        ...
        let claimed_total = leafsii::claim_sp_rewards(&mut protocol, &mut pool, &mut position, ts::ctx(&mut scenario));
        ...
        let (_burned3, _indexed3) = stability_pool::sp_controller_rebalance(&mut pool, &dummy_cap3, 200_000, reward3);
    ...
    fun test_yield_harvest_bounty() {
        ...
        ts::next_tx(&mut scenario, @0x1);
        let protocol = ts::take_shared<Protocol<TEST_FTOKEN, TEST_XTOKEN>>(&scenario);
        let mut pool = ts::take_shared<StabilityPool<TEST_FTOKEN>>(&scenario);
        // Using admin_cap from setup_protocol_with_sp
        ...
        let yield_coin = coin::mint_for_testing<FungibleStakedSui>(yield_amount, ts::ctx(&mut scenario));
        let pool_protocol_id = stability_pool::pool_id(&pool);
        let dummy_cap = stability_pool::create_dummy_capability_with_id(pool_protocol_id, ts::ctx(&mut scenario));
    ...
    fun test_claim_with_no_rewards_fails() {
        ...
        let _claimed = leafsii::claim_sp_rewards(&mut protocol, &mut pool, &mut position, ts::ctx(&mut scenario));
