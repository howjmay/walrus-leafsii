...
module leafsii::test_fee_policy {
    ...
    fun test_normal_mode_fees() {
        ...
        let (reserve_redeemed_f, ticket_opt_f) = leafsii::redeem_f(&mut protocol, &pool, f_redeem, &clock, ts::ctx(&mut scenario));
        option::destroy_none(ticket_opt_f);

        // Test xToken redeem with normal fee (0.5%)
        let x_redeem = coin::mint_for_testing<TEST_XTOKEN>(25_000, ts::ctx(&mut scenario));
        let (reserve_redeemed_x, ticket_opt_x) = leafsii::redeem_x(&mut protocol, &pool, x_redeem, &clock, ts::ctx(&mut scenario));
        option::destroy_none(ticket_opt_x);
    ...
    fun test_l2_user_rebalance_mode_bonuses() {
        ...
        let f_redeem = coin::mint_for_testing<TEST_FTOKEN>(50_000, ts::ctx(&mut scenario));
        let (reserve_redeemed_f, ticket_opt) = leafsii::redeem_f(&mut protocol, &pool, f_redeem, &clock, ts::ctx(&mut scenario));
