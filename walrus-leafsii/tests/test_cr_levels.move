...
module leafsii::test_cr_levels {
    ...
    // Price scaled to 1e9 due to PF_FIXED and SCALE_FACTOR change from 1e6 to 1e9
    const INITIAL_PRICE_E9: u64 = 2_000_000_000; // $2.00 * 1e9 (was 2_000_000 in 1e6)
    const DEPOSIT_AMOUNT: u64 = 1_000_000; // 1 TEST_RESERVE
    const SCALE_FACTOR: u64 = 1_000_000_000; // Updated from 1_000_000 to 1_000_000_000 (1e9)

    // CR Thresholds (from leafsii.move:41-43)
    // Updated to 1e9 scale (PF_FIXED changed from 1e6 to 1e9)
    const CR_T_L1: u64 = 1_306_000_000;  // 1.306 * 1e9 - high CR
    const CR_T_L2: u64 = 1_206_000_000;  // 1.206 * 1e9 - marginal above min
    const CR_T_L3: u64 = 1_144_000_000;  // 1.144 * 1e9 - at min boundary
    ...
    fun setup_protocol_with_sp(scenario: &mut Scenario, clock: &Clock): (Coin<TEST_FTOKEN>, Coin<TEST_XTOKEN>, leafsii::AdminCap) {
        ...
        let (f_coin, x_coin, admin_cap) = leafsii::init_protocol<TEST_FTOKEN, TEST_XTOKEN>(
            ...
            INITIAL_PRICE_E9, // Updated from INITIAL_PRICE_E6
            reserve_coin,
            &mut pool,
            sp_cap,
            clock,
    ...
    fun setup_test(): (Scenario, Clock, MockOracle<SUI>) {
        ...
        let oracle = oracle::create_mock_oracle<SUI>(INITIAL_PRICE_E9, &clock, ctx); // Updated from INITIAL_PRICE_E6
    ...
    fun push_cr_to_level(
        ...
        // fToken amounts scaled down by 1000x due to PF_FIXED change from 1e6 to 1e9
        let f_token = coin::mint_for_testing<TEST_FTOKEN>(3_000, ctx); // Ample fToken (was 3_000_000)
        ...
        if (target_level == 1) {
            ...
            // L2: Need CR between 1.206 and 1.306 (scaled to 1e9)
            // Target CR = 1.25, so reserve_net_usd / (nf * pf) = 1.25
            // reserve_net_usd = nf * pf * 1.25 = nf * 1_000_000_000 * 1.25
            // CR values scaled by 1000x due to PF_FIXED change from 1e6 to 1e9
            let target_cr = 1_250_000_000; // 1.25 * 1e9 (was 1_250_000)
            let target_reserve_net_usd = math::mul_div(current_nf, target_cr, SCALE_FACTOR);
            let current_reserve_usd = math::mul_div(current_reserve, INITIAL_PRICE_E9, SCALE_FACTOR); // Updated from INITIAL_PRICE_E6
            if (current_reserve_usd > target_reserve_net_usd) {
                let needed_obligation_usd = current_reserve_usd - target_reserve_net_usd;
                let needed_obligation_r = math::mul_div(needed_obligation_usd, SCALE_FACTOR, INITIAL_PRICE_E9); // Updated from INITIAL_PRICE_E6
                ...
                // fToken amounts scaled down by 1000x (was 800_000)
                let (_burned, _indexed) = stability_pool::sp_controller_rebalance(pool, &dummy_cap, 800, needed_obligation_r);
                stability_pool::destroy_capability(dummy_cap);
            };
        } else if (target_level == 3) {
            // L3: Need CR between 1.144 and 1.206 (scaled to 1e9)
            // CR values scaled by 1000x due to PF_FIXED change from 1e6 to 1e9
            let target_cr = 1_175_000_000; // 1.175 * 1e9 (was 1_175_000)
            let target_reserve_net_usd = math::mul_div(current_nf, target_cr, SCALE_FACTOR);
            let current_reserve_usd = math::mul_div(current_reserve, INITIAL_PRICE_E9, SCALE_FACTOR); // Updated from INITIAL_PRICE_E6
            if (current_reserve_usd > target_reserve_net_usd) {
                let needed_obligation_usd = current_reserve_usd - target_reserve_net_usd;
                ...
                let pool_protocol_id = stability_pool::pool_id(pool);
                let dummy_cap = stability_pool::create_dummy_capability_with_id(pool_protocol_id, ctx);
                // fToken amounts scaled down by 1000x (was 1_000_000)
                let (_burned, _indexed) = stability_pool::sp_controller_rebalance(pool, &dummy_cap, 1_000, needed_obligation_r);
                stability_pool::destroy_capability(dummy_cap);
            };
        } else if (target_level == 4) {
            // L4: Need CR between 1.050 and 1.144 (scaled to 1e9)
            // CR values scaled by 1000x due to PF_FIXED change from 1e6 to 1e9
            let target_cr = 1_100_000_000; // 1.1 * 1e9 (was 1_100_000)
            let target_reserve_net_usd = math::mul_div(current_nf, target_cr, SCALE_FACTOR);
            let current_reserve_usd = math::mul_div(current_reserve, INITIAL_PRICE_E9, SCALE_FACTOR); // Updated from INITIAL_PRICE_E6
            if (current_reserve_usd > target_reserve_net_usd) {
                let needed_obligation_usd = current_reserve_usd - target_reserve_net_usd;
                ...
                let pool_protocol_id = stability_pool::pool_id(pool);
                let dummy_cap = stability_pool::create_dummy_capability_with_id(pool_protocol_id, ctx);
                // fToken amounts scaled down by 1000x (was 1_200_000)
                let (_burned, _indexed) = stability_pool::sp_controller_rebalance(pool, &dummy_cap, 1_200, needed_obligation_r);
                stability_pool::destroy_capability(dummy_cap);
            };
        };
    ...
    fun test_cr_level_1_high_cr() {
        ...
        // fToken amounts scaled down by 1000x due to PF_FIXED change from 1e6 to 1e9
        let redeem_f = coin::mint_for_testing<TEST_FTOKEN>(500, ts::ctx(&mut scenario)); // Was 500_000
        let (redeemed_reserve, ticket_opt) = leafsii::redeem_f(&mut protocol, &pool, redeem_f, &clock, ts::ctx(&mut scenario));
    ...
    fun test_cr_level_3_at_min_boundary() {
        ...
        // CR values scaled by 1000x due to PF_FIXED change from 1e6 to 1e9
        let target_cr = 1_300_000_000; // Target 1.3 CR * 1e9 (was 1_300_000)
        leafsii::protocol_rebalance_l3_to_target(&mut protocol, &mut pool, target_cr, &admin_cap, ts::ctx(&mut scenario));
    ...
    fun test_cr_level_4_below_min() {
        ...
        // CR values scaled by 1000x due to PF_FIXED change from 1e6 to 1e9
        let target_cr = 1_200_000_000; // Target back to L1 * 1e9 (was 1_200_000)
        leafsii::protocol_rebalance_l3_to_target(&mut protocol, &mut pool, target_cr, &admin_cap, ts::ctx(&mut scenario));
    ...
    fun test_redeem_f_allowed_at_l1() {
        ...
        // fToken amounts scaled down by 1000x due to PF_FIXED change from 1e6 to 1e9
        let f_coin = coin::mint_for_testing<TEST_FTOKEN>(100, ts::ctx(&mut scenario)); // Was 100_000
        let (_redeemed, ticket_opt) = leafsii::redeem_f(&mut protocol, &pool, f_coin, &clock, ts::ctx(&mut scenario));
        option::destroy_none(ticket_opt);
    ...
    fun test_redeem_x_allowed_at_l1() {
        ...
        let (_redeemed, ticket_opt) = leafsii::redeem_x(&mut protocol, &pool, x_coin, &clock, ts::ctx(&mut scenario));
        option::destroy_none(ticket_opt);
    ...
    fun test_cr_calculation_accuracy() {
        ...
        // F tokens: 1.0 F at $1.00 = $1.0 USD (nf scaled down by 1000x due to PF_FIXED 1e9)
        // Expected CR = 2.0M / 1.0 = 2.0 = 2_000_000_000 in scaled units (1e9)
        // CR values scaled by 1000x due to PF_FIXED change from 1e6 to 1e9
        let cr = leafsii::collateral_ratio(&protocol, &pool);
        assert!(cr == 2_000_000_000, 0); // Was 2_000_000, now 2_000_000_000
        ...
        // New state with fee policy:
        // After minting 500k reserve with 0.5% fee:
        //   Net deposit: 497_500 (500k - 2.5k fee)
        //   New reserve: 1_497_500 at $2.00 = $2.995M USD
        //   F minted: 497_500 * 2_000_000_000 / 1_000_000_000 = 995_000 units (0.995 F)
        //   Total F: 1_000_000 + 995_000 = 1_995_000 units (1.995 F) at $1.00 each
        // CR = (1_497_500 * 2_000_000_000 * 1_000_000_000) / (1_995_000 * 1_000_000_000)
        //    = 2_995_000_000_000_000_000 / 1_995_000_000_000_000
        //    = 1_501_253_132... ≈ 1_501_253_132 in 1e9 scale
        // CR values scaled by 1000x due to SCALE_FACTOR change from 1e6 to 1e9
        let new_cr = leafsii::collateral_ratio(&protocol, &pool);
        assert!(new_cr == 1_501_253_132, 1); // Was 1_501_253 in 1e6, now 1_501_253_132 in 1e9

        // Verify level calculation
        let level = leafsii::current_level(&protocol, &pool);
        assert!(level == 0, 2); // 1.5 * 1e9 >= CR_T_L1 (1.306 * 1e9) = Normal mode
