...
module leafsii::test_multi_user_liquidation {
    ...
    fun test_three_actor_liquidation_scenario() {
        ...
        let actor_a_reserve = coin::mint_for_testing<SUI>(400_000, ts::ctx(&mut scenario)); // More conservative amount
        ...
        let mut actor_b_position = stability_pool::create_position<TEST_FTOKEN>(ts::ctx(&mut scenario));
        ...
        let mut c_user_position = stability_pool::create_position<TEST_FTOKEN>(ts::ctx(&mut scenario));
        ...
        let b_claimed_rewards = leafsii::claim_sp_rewards(&mut protocol, &mut pool, &mut actor_b_position, ts::ctx(&mut scenario));
    ...
    fun test_sequential_liquidations_multiple_users() {
        ...
        // Add extra reserve
        let extra_reserve = coin::mint_for_testing<SUI>(2_000_000, ts::ctx(&mut scenario));
        ...
        let mut user1_position = stability_pool::create_position<TEST_FTOKEN>(ts::ctx(&mut scenario));
        let mut user2_position = stability_pool::create_position<TEST_FTOKEN>(ts::ctx(&mut scenario));
        let mut user3_position = stability_pool::create_position<TEST_FTOKEN>(ts::ctx(&mut scenario));
        ...
        let pool_protocol_id = stability_pool::pool_id(&pool);
        let dummy_cap = stability_pool::create_dummy_capability_with_id(pool_protocol_id, ts::ctx(&mut scenario));
        let (_burned1, _indexed1) = stability_pool::sp_controller_rebalance(&mut pool, &dummy_cap, 600_000, liquidation1_rewards);
        stability_pool::destroy_capability(dummy_cap);
        ...
        let user2_claim1 = leafsii::claim_sp_rewards(&mut protocol, &mut pool, &mut user2_position, ts::ctx(&mut scenario));
        ...
        let pool_protocol_id = stability_pool::pool_id(&pool);
        let dummy_cap = stability_pool::create_dummy_capability_with_id(pool_protocol_id, ts::ctx(&mut scenario));
        let (_burned2, _indexed2) = stability_pool::sp_controller_rebalance(&mut pool, &dummy_cap, 700_000, liquidation2_rewards);
        stability_pool::destroy_capability(dummy_cap);
        ...
        let user1_final_claim = leafsii::claim_sp_rewards(&mut protocol, &mut pool, &mut user1_position, ts::ctx(&mut scenario));
        let user2_final_claim = leafsii::claim_sp_rewards(&mut protocol, &mut pool, &mut user2_position, ts::ctx(&mut scenario));
        let user3_final_claim = leafsii::claim_sp_rewards(&mut protocol, &mut pool, &mut user3_position, ts::ctx(&mut scenario));
    ...
    fun test_liquidation_with_protocol_rebalance() {
        ...
        let pool_protocol_id = stability_pool::pool_id(&pool);
        let dummy_cap = stability_pool::create_dummy_capability_with_id(pool_protocol_id, ts::ctx(&mut scenario));
        let (_burned, _indexed) = stability_pool::sp_controller_rebalance(&mut pool, &dummy_cap, 100_000, 50_000);
        stability_pool::destroy_capability(dummy_cap);
    ...
    fun test_liquidation_edge_case_small_deposits() {
        ...
        // Add some reserve
        let extra_reserve = coin::mint_for_testing<SUI>(500_000, ts::ctx(&mut scenario));
        let _extra_minted = leafsii::mint_f(&mut protocol, &pool, extra_reserve, ts::ctx(&mut scenario));
        coin::burn_for_testing(_extra_minted);
        ...
        let pool_protocol_id = stability_pool::pool_id(&pool);
        let dummy_cap = stability_pool::create_dummy_capability_with_id(pool_protocol_id, ts::ctx(&mut scenario));
        let (_burned, indexed) = stability_pool::sp_controller_rebalance(&mut pool, &dummy_cap, 500, small_reward);
        stability_pool::destroy_capability(dummy_cap);
        ...
        if (rewards1 > 0) {
            let claim1 = leafsii::claim_sp_rewards(&mut protocol, &mut pool, &mut user1_position, ts::ctx(&mut scenario));
            assert!(coin::value(&claim1) == rewards1, 4);
            transfer::public_transfer(claim1, @0x1);
        };
        
        if (rewards2 > 0) {
            let claim2 = leafsii::claim_sp_rewards(&mut protocol, &mut pool, &mut user2_position, ts::ctx(&mut scenario));
